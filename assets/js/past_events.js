...
function Search(categorias, textoingresado) {
    ...
    if (categorias.length > 0 && textoingresado == "") {
        pastEvents.filter(event => categorias.includes(event.category)).forEach(event => {
        ...
        pastEvents.filter(event => categorias.includes(event.category)).filter(event => event.name.toLowerCase().includes(textoingresado) || event.description.toLowerCase().includes(textoingresado)).forEach(event => { htmlResultados += createCard(event) });

        console.log(htmlResultados);


    } else if (categorias.length == 0 && textoingresado != "") {
        pastEvents.filter(event => event.name.toLowerCase().includes(textoingresado) || event.description.toLowerCase().includes(textoingresado)).forEach(event => { htmlResultados += createCard(event) });



    } else if (categorias.length == 0 && cardsearch.length == 0) {
        pastEvents.forEach(event => { htmlResultados += createCard(event) });
    }
