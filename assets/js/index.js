...
function Search(categorias, textoingresado) {
    ...
    if (categorias.length > 0 && textoingresado == "") {
        ...
        data.events.filter(event => categorias.includes(event.category)).filter(event => event.name.toLowerCase().includes(textoingresado) || event.description.toLowerCase().includes(textoingresado)).forEach(event => { htmlResultados += createCard(event) });

        console.log(htmlResultados);

    } else if (categorias.length == 0 && textoingresado != "") {
        data.events.filter(event => event.name.toLowerCase().includes(textoingresado) || event.description.toLowerCase().includes(textoingresado)).forEach(event => { htmlResultados += createCard(event) });
